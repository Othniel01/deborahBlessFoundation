...
export default function Numbers() {
  ...
  const [workshopsHeld, setWorkshopsHeld] = useState(0);
  ...
  useEffect(() => {
    if (inView) {
      ...
      if (childrenSupported === 0)
        animateCount(20000000, 2000, setChildrenSupported); // Children Supported
      if (workshopsHeld === 0) animateCount(80000, 2000, setWorkshopsHeld); // Workshops Held
      if (toolsDistributed === 0) animateCount(100, 2000, setToolsDistributed); // Tools Distributed
      if (countriesWorldwide === 0)
        animateCount(10, 1000, setCountriesWorldwide); // Countries Worldwide
    }
  }, [
    inView,
    childrenSupported,
  ...
  return (
    ...
    >
      <div className="text-white">
        <h1 className="min-[962px]:text-4xl text-2xl">
          {childrenSupported.toLocaleString()}
        </h1>
